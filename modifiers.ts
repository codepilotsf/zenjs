...
export function parseModifiers(dom, ctx) {
  ...
  zActiveEls.forEach((el) => {
    ...
    if (zActiveValue === "@") {
      ...
      isFalsey(zActiveValue)
        ? el.classList.remove("active")
        : el.classList.add("active");
      el.setAttribute("z-active", zActiveValue ? "true" : "false");
    }
  });
  ...
  zInvalidEls.forEach((el) => {
    ...
    el.setAttribute("z-invalid", zInvalidValue ? "true" : "false");
  });
  ...
  zDisabledEls.forEach((el) => {
    ...
    el.setAttribute("z-disabled", zDisabledValue ? "true" : "false");
  });
  ...
  zCheckedEls.forEach((el) => {
    ...
    el.setAttribute("z-checked", zCheckedValue ? "true" : "false");
  });
  ...
  zSelectedEls.forEach((el) => {
    ...
    el.setAttribute("z-selected", zSelectedValue ? "true" : "false");
  });
    ...
    modifiers.forEach((modifier) => {
      switch (modifier.type) {
        ...
        case "removeClass":
          ...
          break;
        case "setAttr":
          dom.getElementById(id)?.setAttribute(
            modifier.data[0],
            modifier.data[1],
          );
          break;
