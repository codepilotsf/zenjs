...
export async function getNearest(absPath, targetTemplate, isRecursing = false) {
  // When initially invoked and no file extension, try using the absPath as parentDir.
  const parentDir =
    !isRecursing && !path.extname(absPath) ? absPath : path.dirname(absPath);
  ...
  const reachedTop =
    path.normalize(parentDir) == path.normalize(routesDir) ||
